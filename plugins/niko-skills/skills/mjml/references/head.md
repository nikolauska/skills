...
## mj-style
Adds custom CSS. Use `inline="inline"` to force Juice inlining when a rule must survive clients that strip head CSS.
...
**Compatibility rule**: Critical layout styles should use component attributes. Client support for head CSS varies, so inline essential declarations.
