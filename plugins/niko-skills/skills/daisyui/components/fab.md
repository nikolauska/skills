...
- Prefer `<button type="button">` for every interactive control. If a non-native control is unavoidable, implement Enter and Space activation
- Give every icon-only button an accessible name with visible text or `aria-label`; a tooltip alone is not an accessible name
