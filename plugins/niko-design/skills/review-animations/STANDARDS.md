...
- **Prefer `transform` and `opacity`** because browsers can often composite them without layout or paint. Confirm in browser performance tooling; promotion and compositor execution are not guaranteed.
- **Treat layout-property animation as a measured tradeoff.** `padding`/`margin`/`height`/`width`/`top`/`left` can trigger layout and paint, but intrinsic-size transitions may require them. Flag sustained or large layout work, not every occurrence.
- **Scope per-frame writes narrowly.** A CSS variable on a parent can invalidate descendants; write to the animated leaf when a performance trace shows recalculation cost.
- **Do not infer acceleration from Motion syntax.** Shorthand values and full `transform` strings can both be updated by per-frame JavaScript. Profile before rewriting an API solely for performance.
- **Prefer CSS or WAAPI for predetermined motion** because browsers may optimize qualifying animations without per-frame JavaScript. Use JavaScript or springs for dynamic, interruptible, or gesture-driven values.
- **WAAPI** provides programmatic, interruptible browser animation without another library:
  ```js
  element.animate(
