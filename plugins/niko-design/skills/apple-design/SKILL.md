---
name: apple-design
description: Translates Apple's interface-design and physical-motion principles to the web. Use when building or reviewing gesture-driven UI, springs, drag/swipe/sheet interactions, momentum, interruptible transitions, translucent materials, typography, reduced motion, feedback, or spatial consistency; do not use for native SwiftUI or AppKit implementation.
---
...
- **Keep reversible paths continuous.** Re-target from the current presentation value and carry velocity when possible; choose direction-appropriate easing and verify rapid reversals in slow motion.
...
- Prefer compositor-friendly properties such as `transform` and `opacity`, verify in performance tooling, and apply `will-change` only shortly before expensive motion when measurement shows it helps.
...
| Reversible transition | Re-target from the live value | preserve velocity and test rapid reversals |
| Decide reverse vs. commit | Use projected endpoint | combine release position and velocity |
