...
description: Searches a codebase or UI for places that would genuinely benefit from motion and rejects candidates that should remain static. Use for requests such as "what could be animated here?" or "make this feel more alive"; it is read-only and does not review or fix existing animations.
---
...
5. **Keep analysis local and safe.** Never read secrets, credentials, private keys, or `.env` files; do not browse external sites, install packages, run mutating commands, or call external systems.
...
| --- | --- | --- | --- | --- | --- |
| 1 | `Toast.tsx:41` | New toasts appear instantly | Preventing a jarring change | Occasional | Enter via `@starting-style`: `opacity: 0; translateY(100%)` → settled, `transition: transform 200ms var(--ease-out), opacity 200ms var(--ease-out)`, exit same edge |
...
Every "Suggested motion" cell carries exact properties, duration, and easing. Reuse the target repository's motion tokens. If none exist, use the catalog fallbacks (`cubic-bezier(0.23, 1, 0.32, 1)` for ease-out, `cubic-bezier(0.77, 0, 0.175, 1)` for ease-in-out, `cubic-bezier(0.32, 0.72, 0, 1)` for drawers) and state that the implementation plan must place any shared token alongside existing design tokens. Prefer `transform` and `opacity`; justify intrinsic-size or other paint/layout exceptions. Include reduced-motion handling and `@media (hover: hover) and (pointer: fine)` gating for hover motion.
