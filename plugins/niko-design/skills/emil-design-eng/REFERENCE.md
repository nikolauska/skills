...
Browsers can often composite `transform` and `opacity` without layout or paint. Treat this as a preference, not a guarantee: verify in performance tooling, and allow measured intrinsic-size transitions when layout change is the required behavior.

### CSS variables are inheritable

Changing a CSS variable on a parent can invalidate styles for descendants. In a hot drag loop, prefer writing the animated leaf's `transform` directly when a performance trace shows recalculation cost.

```js
// Broader invalidation risk on a parent
element.style.setProperty('--swipe-amount', `${distance}px`);

// Narrow write to the animated leaf
element.style.transform = `translateY(${distance}px)`;
```

### Motion-library performance

Do not infer hardware acceleration from whether a library uses shorthand values or a full `transform` string. Either form can involve per-frame JavaScript, while the resulting transform may still be composited. Profile the actual interaction before rewriting syntax solely for performance.

### CSS and WAAPI for predetermined motion

For predetermined transitions, prefer CSS or the Web Animations API because browsers may optimize qualifying animations without per-frame JavaScript. Use JavaScript or springs when values are dynamic, interruptible, or gesture-driven. No API guarantees smoothness: verify under representative load.
...
Step through animations frame by frame with the browser's available animation tooling. This reveals timing issues between coordinated properties that are hard to see at full speed.

### Test on real devices

For touch interactions such as drawers and swipe gestures, test on representative physical devices and use the platform's remote debugging tools when available. Emulators help with layout but do not reproduce real gesture feel.
