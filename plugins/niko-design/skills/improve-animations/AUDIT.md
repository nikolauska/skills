...
- **Prefer `transform` and `opacity`** because browsers can often composite them without layout or paint. Verify in browser tooling; compositing is not guaranteed.
- **Treat layout-property animation as a risk, not an automatic failure.** Flag sustained or large `width`/`height`/`margin`/`padding`/`top`/`left` animation when it threatens the frame budget; allow measured intrinsic-size transitions when layout change is the behavior.
- **`transition: all`** animates unintended properties and should be replaced with an explicit property list.
- **Do not infer acceleration from API syntax.** Motion-library shorthands and full `transform` strings can both involve per-frame JavaScript. Profile the interaction before recommending a rewrite.
- **Prefer CSS or WAAPI for predetermined motion** because browsers may optimize qualifying animations independently of per-frame JavaScript. Use JavaScript or springs when values are dynamic or gesture-driven.
- **Scope high-frequency updates narrowly.** Parent CSS variables can invalidate descendants; set a leaf transform directly when profiling shows recalculation cost.
- Keep transition-time `filter: blur()` small and verify it on target devices; filters can be expensive.
