...
- **Status**: TODO
- **Commit**: <short Git commit when available; otherwise `unavailable (not a Git worktree)`>
...
- **Feel check**: run the UI, trigger <interaction>, and confirm:
  ...
  - In available browser animation tooling, slow playback and confirm <detail>.
  - Emulate `prefers-reduced-motion` with browser tooling or an OS setting and confirm movement is dropped but opacity feedback remains.
...
- Use exact values from [AUDIT.md](AUDIT.md), respecting the target repo's existing tokens; never approximate from memory.
