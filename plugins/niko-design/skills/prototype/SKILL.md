---
name: prototype
description: Builds multiple genuinely different versions of one UI component behind a live visual picker, then promotes the user's selected variant. Use only when explicitly asked to prototype or compare UI directions; do not use for UI review, animation audits, or dependency selection.
---
...
Divergence is not an excuse to drop the craft bar. Every variant must use purposeful, frequency-appropriate motion, correct origins, reduced-motion handling, and compositor-friendly properties where practical. Exceptions such as a small picker highlight must be explicit and measured.
...
8. **Preserve safety and accessibility.** Confirm destructive integration changes; retain keyboard operation, focus visibility, semantic controls, and reduced-motion behavior.
